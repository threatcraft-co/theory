...
def run(
    ...
    **kwargs: Any,
    ...
    if output == "json":
        ...
        _output_exec(profile, save, sector=kwargs.get("sector", ""))
    elif output == "all":
...
def _output_exec(profile: dict[str, Any], save: bool, sector: str = "") -> None:
    """
    Generate and display a non-technical executive summary (BLUF).
    Optionally sector-aware: --sector energy focuses the risk paragraph
    on what this actor means for energy sector organizations.
    """
    try:
        from collectors.intelligence_synthesizer import (
            IntelligenceSynthesizer, load_provider,
        ...
        print("[theory] Rich or synthesizer not available.", file=sys.stderr)
        return

    provider = load_provider()
    if not provider or not provider.available:
        print(
            "[theory] --output exec requires an LLM provider. "
            "Set ANTHROPIC_API_KEY, OPENAI_API_KEY, or start Ollama in .env.",
            file=sys.stderr,
        )
        return
    ...
    console.print()
    console.print(f"[bold cyan]Generating executive summary for {actor_name}…[/]")
    console.print()

    synth   = IntelligenceSynthesizer(provider)
    summary = synth.synthesize_executive_summary(
        profile      = profile,
        queried_name = actor_name,
        sector       = sector,
    )

    if not summary:
        console.print("[red]Executive summary generation failed — check your LLM provider config.[/]")
        return

    # Render as a clean panel
    sector_label = f" | Sector: {sector}" if sector else ""
    title = f"[bold]Executive Summary — {actor_name}[/]{sector_label}"
    console.print(Panel(summary, title=title, border_style="cyan", padding=(1, 2)))
    console.print()
    console.print(
        f"[dim]Generated by {provider.name} · "
        f"Sources: {', '.join(profile.get('sources_cited', []))}[/]"
    )
    console.print()
    ...
    if save:
        ...
        slug = re.sub(r"[^a-z0-9]", "_", actor_name.lower())
        sector_suffix = f"_{re.sub(r'[^a-z0-9]', '_', sector.lower())}" if sector else ""
        path = output_dir / f"{slug}{sector_suffix}_exec.md"
        lines = [
            f"# Executive Summary — {actor_name}",
            f"> Generated by THEORY · {provider.name} · "
            f"Sources: {', '.join(profile.get('sources_cited', []))}",
            "",
        ]
        if sector:
        ...
        console.print(f"[dim][theory] Executive summary saved → {path}[/]")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--output", "-o",
        choices=["dossier", "json", "stix", "csv", "all", "exec"],
        default="dossier",
        metavar="FORMAT",
        help=(
            "Output format. "
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Focuses the executive summary risk paragraph on your industry. "
